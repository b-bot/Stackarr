...
default_stackarr_database_file() {
    ...
    # Legacy databases are migration inputs, never implicit runtime defaults.
    if [[ -n "${STACKARR_LEGACY_DATABASE_FILE:-}" && -f "$STACKARR_LEGACY_DATABASE_FILE" ]]; then
        printf '%s\n' "$STACKARR_LEGACY_DATABASE_FILE"
        return 0
    fi
...
load_env() {
    ...
    configure_docker_environment
    # Load installed paths before choosing the database. A source checkout may
    # still contain an old database and must not become the controller's mount.
    load_compose_runtime_env
    ...
    : "${TDARR_CACHE_ROOT:=$APP_ROOT/cache/tdarr}"
    : "${TDARR_MEDIA_ROOT:=$MEDIA_ROOT}"
    if flag_enabled "$ENABLE_TDARR" && [[ -z "$TDARR_API_KEY" ]]; then
        TDARR_API_KEY="tapi_$(random_hex_secret 24)"
        set_env_value TDARR_API_KEY "$TDARR_API_KEY"
    fi
